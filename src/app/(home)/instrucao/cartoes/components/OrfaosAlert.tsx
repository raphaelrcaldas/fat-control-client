...
import type { CartaoOrfaoPublic } from "services/routes/instrucao/cartoes";
import { useToast } from "@/app/context/toast";
import { useCartoesOrfaos, useDeleteCartoesOrfaos } from "@/hooks/queries";
...
function CleanupModal({ show, onClose, itens }: CleanupModalProps) {
   const { push } = useToast();
   const deleteMutation = useDeleteCartoesOrfaos();
      ...
      try {
         ...
         push({
            message: result.deleted
               ? `${result.deleted} cartão(ões) removido(s)`
   ...
   return (
      <Modal show={show} onClose={onClose} size="2xl" dismissible>
         <ModalHeader>Limpar cartões de militares inativos</ModalHeader>
         <ModalBody>
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">
               Selecione os militares. A exclusão remove o cartão de instrução
               (idiomas/CVI) de militares atualmente inativos.
            </p>
...
export default function OrfaosAlert() {
   const { data: orfaos, isLoading } = useCartoesOrfaos();
      ...
      <>
         <Alert color="warning" icon={HiExclamation} withBorderAccent>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
               <span>
                  <strong>{orfaos.total_registros}</strong> cartão(ões) de
                  instrução de militar(es) inativo(s).
               </span>
               <Button
                  color="yellow"
                  size="xs"

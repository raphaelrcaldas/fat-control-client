            ...
            <form onSubmit={handleSubmit} className='space-y-4'>
               ...
               <div className='grid grid-cols-2 gap-4'>
                  <div className='flex flex-col gap-2'>
                     ...
                     >
                        {isFuncAvailable("pil", trip.funcs || [], editingFunc.id) && (
                           <option value='pil'>PIL - Piloto</option>
                        )}
                        {isFuncAvailable("mc", trip.funcs || [], editingFunc.id) && (
                           <option value='mc'>MC - Mecânico</option>
                        )}
                        {isFuncAvailable("lm", trip.funcs || [], editingFunc.id) && (
                           <option value='lm'>LM - Loadmaster</option>
                        )}
                        {isFuncAvailable("oe", trip.funcs || [], editingFunc.id) && (
                           <option value='oe'>
                              OE - Operador de Equipamentos
                           </option>
                        )}
                        {isFuncAvailable("os", trip.funcs || [], editingFunc.id) && (
                           <option value='os'>OS - Observador SAR</option>
                        )}
                        {isFuncAvailable("tf", trip.funcs || [], editingFunc.id) && (

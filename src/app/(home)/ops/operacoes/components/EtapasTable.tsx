      ...
      <section className="rounded border border-slate-300 bg-white shadow">
         <header className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
            ...
            <div className="flex items-center gap-2">
               <span className="rounded-full bg-slate-100 px-2.5 py-1 font-mono text-xs font-semibold text-slate-600 tabular-nums">
                  {rows.length} de {etapas.length} · Σ{" "}
                  {minutesToTime(totalTvoo)}
               </span>
               <PermBased resource="operacoes.etapa" requiredPerm="create">
                  <Button color="primary" size="xs" onClick={onAssociar}>
                     ...
                     {rows.map((e) => (
                        <TableRow key={e.id} className="bg-white">
                           ...
                           <TableCell className="text-center">
                              {e.esforco ? (
                                 <span className="text-primary-700 px-1.5 py-0.5 text-[11px]">
                                    {e.esforco}
                                 </span>
                              ) : (
                                 <span className="text-slate-300">—</span>

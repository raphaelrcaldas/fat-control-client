...
export function EtapasTableSkeleton({ rows = 6 }: { rows?: number }) {
   return (
      <section className="overflow-x-auto rounded border border-slate-200 bg-white shadow-sm motion-safe:animate-pulse">
         ...
         {/* Cabeçalho da tabela */}
         <div className="grid min-w-[620px] grid-cols-7 gap-2 border-b border-slate-100 px-4 py-2.5">
         ...
         <div className="divide-y divide-slate-100">
            {Array.from({ length: rows }).map((_, i) => (
               <div
                  key={i}
                  className="grid min-w-[620px] grid-cols-7 items-center gap-2 px-4 py-3"
               >

...
export async function gerarRelatorio(comiss: ComissWithMiss) {
   ...
   missoes.forEach((m, index) => {
      ...
      const regres = new Date(
         afast.getTime() + daysToSum * 24 * 60 * 60 * 1000
      );
      ...
      m.pernoites.forEach((p) => {
         ...
         listPntsStrCids.push(
            `${p.cidade.nome}-${p.cidade.uf} (${diarias.toFixed(1)})`
         );

         let pntVals = vals.map((val) => `(${val.qtd}*${val.valor})`).join("+");
      ...
      valDiaria.value = listPntsStrFml.length
         ? { formula: listPntsStrFml.join("+") }
         : 0;

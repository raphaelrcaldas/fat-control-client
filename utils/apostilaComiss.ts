...
export async function gerarRelatorioDocx(comiss: ComissWithMiss): Promise<Blob> {
    try {
        // Carregar o template
        ...
        const nameLabel = `${user.p_g} ${user.esp} ${user.nome_completo} (${user.saram})`.toUpperCase()
        ...
        doc.setData({ name: nameLabel, total_dias: comiss.dias_comp, modulos: modulos.join('\n') });
